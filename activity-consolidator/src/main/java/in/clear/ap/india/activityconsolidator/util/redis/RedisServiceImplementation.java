...
public class RedisServiceImplementation implements RedisService {
    ...
    @Override
    public Map<String, RedisFileStatusValue> getAllValuesFromHash(String hashName) {
        ReactiveHashOperations<String,String,RedisFileStatusValue> reactiveHashOperations = redisTemplate.opsForHash();
        return reactiveHashOperations.entries(hashName).collectMap(Map.Entry::getKey, Map.Entry::getValue).share().block();
    }
